// Fitur 1: Riwayat Pembayaran (Customer)
...
export default function PaymentHistory() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => getPaymentHistoryApi({ page, search, per_page: 15 }).then(r => r.data),
      ...
      <div className="max-w-3xl mx-auto px-4 py-8">
            ...
            {histories.map(h => (
              <div key={h.id} className="card p-4 flex items-center justify-between gap-4">
                <div className="flex-1 min-w-0">
                  <p className="font-mono text-sm font-semibold text-purple-600 truncate">{h.booking_code}</p>
                  <p className="text-sm text-gray-500 mt-0.5">{h.field_name || h.description}</p>
        ...
        {meta.last_page > 1 && (
          <div className="flex items-center justify-center gap-3 mt-6">
            ...
            <span className="text-sm text-gray-500">Halaman {page} / {meta.last_page}</span>
            <button
